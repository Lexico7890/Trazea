...
const StatusBadge = ({
  ...
  state: string;
  type: "default" | "success" | "warning" | "destructive" | "secondary";
}) => {
  const colorMap = {
  ...
  return (
    <span
      className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wide ${colorMap[type] || colorMap.default} border`}
    >
      {state}
    </span>
  );
      ...
      <div className="flex justify-between items-start mb-6">
        <div className="flex gap-4 items-center">
          ...
          <div>
            ...
            <p className="text-gray-500 text-xs uppercase font-bold tracking-wider mt-0.5">
              {event.operacion}
            </p>
          </div>
        </div>
        <div className="flex flex-col items-end gap-2">
          <div className="bg-[#252836] px-3 py-1.5 rounded-full border border-[#2A2E3B]">
            <span className="text-xs text-gray-400 font-medium">
              {format(new Date(event.fecha), "EEE, d MMM • h:mm a", {
                locale: es,
              })}
            </span>
          </div>
          <StatusBadge
            state={event.estado}
            type={
              ["COMPLETADO", "CUADRADO", "DISPONIBLE"].includes(event.estado)
                ? "success"
                : ["PENDIENTE", "EN PROCESO"].includes(event.estado)
                  ? "warning"
                  : ["DESCARGADO", "AGOTADO", "FALTANTE"].includes(event.estado)
                    ? "destructive"
                    : "secondary"
            }
          />
...
const MovimientoTecnicoCard = ({ event }: { event: TimelineEvent }) => {
  const cantidadMatch =
    event.informacion_adicional?.match(/Cantidad:\s*(-?\d+)/i);
    ...
    <BaseEventCard event={event} icon={Wrench}>
      <div>
        ...
        <div className="flex items-baseline gap-1">
          <p
            className={`text-2xl font-bold ${cantidad && cantidad > 0 ? "text-green-500" : "text-pink-500"}`}
          >
            {cantidad
              ? cantidad > 0
                ? `+${cantidad}`
      ...
      <div>
        ...
        <p className="text-base text-white font-mono bg-[#252836] px-2 py-1 rounded inline-block border border-[#2A2E3B]">
          {event.orden || "N/A"}
        </p>
      </div>
    </BaseEventCard>
...
export function MovementHistoryPage() {
  ...
  const { data: repuesto, isLoading: isLoadingRepuesto } =
    useRepuestoByReferencia(referencia || "", currentLocation?.id_localizacion || "");
  const {
    data,
    fetchNextPage,
    hasNextPage,
          ...
          <div className="mb-6 flex flex-row items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground mt-1">
                Registro detallado de transacciones de stock
              </p>
            </div>
