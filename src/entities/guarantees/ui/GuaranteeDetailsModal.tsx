...
export default function GuaranteeDetailsModal({
  open,
  onOpenChange,
  guarantee,
}: GuaranteeDetailsModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 pr-8">
            <div className="flex-1">
              <div className="flex items-center justify-between gap-3">
                ...
                <Badge className={`text-sm shadow-sm ${getStatusColor(guarantee.estado)}`}>
                  {guarantee.estado || "Pendiente"}
                </Badge>
              </div>
            ...
            <section>
              ...
              <div className="bg-muted/30 p-4 rounded-lg border space-y-2">
                <div>
                  ...
                  <p className="font-semibold">{guarantee.nombre_repuesto}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground uppercase font-medium">Referencia</p>
                  <p className="font-mono text-sm">{guarantee.referencia_repuesto}</p>
                </div>
              </div>
            </section>
              ...
              <div className="bg-muted/30 p-4 rounded-lg border space-y-2">
                <div className="flex justify-between">
                  <div>
                    ...
                    <p className="text-sm font-medium">{guarantee.taller_origen}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-muted-foreground uppercase font-medium">Fecha Reporte</p>
                    <p className="text-sm font-medium">
                      {guarantee.fecha_reporte
                        ? format(new Date(guarantee.fecha_reporte), "PPP p", { locale: es })
                        : "-"}
                    </p>
                  </div>
                </div>
              ...
              <div className="bg-muted/30 p-4 rounded-lg border space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{guarantee.reportado_por}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium">{guarantee.tecnico_responsable || "No asignado"}</p>
                  </div>
                </div>
              </div>
            </section>
          ...
          <div className="space-y-6">
            ...
            <section>
              ...
              <div className="relative aspect-video rounded-lg border overflow-hidden bg-muted group">
                {guarantee.url_evidencia_foto ? (
                  <>
                    <img
                      src={guarantee.url_evidencia_foto}
                      alt="Evidencia de garantía"
                      className="object-cover w-full h-full transition-transform duration-300 group-hover:scale-105"
                    />
                    <a
                      href={guarantee.url_evidencia_foto}
                      target="_blank"
        ...
        {guarantee.comentarios_resolucion && (
          <>
